...
export default function MenuScreen() {
  ...
  const openDeviceSettings = async () => {
    try {
      ...
      Alert.alert("Unable to open Settings", "Open your phone Settings app manually.");
    }
  };

  const applyCoupon = () => {
    if (!coupon.trim()) return Alert.alert("Coupon", "Enter a coupon code first.");
    if (coupon.trim().toUpperCase() === "WELCOME10") {
      Alert.alert("Coupon applied ✅", "10% off will be applied at checkout (demo).");
      setCoupon("");
    } else {
  ...
  return (
    <SafeAreaView style={[styles.safe, bg]}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
        {/* Device settings */}
        <Pressable onPress={openDeviceSettings} style={[styles.row, styles.card, cardBg]}>
          <Ionicons name="cog-outline" size={22} color={mut.color as string} />
          <Text style={[styles.rowText, fg]}>Open device settings</Text>
          <Ionicons name="chevron-forward" size={20} color={mut.color as string} />
        </Pressable>
        {/* Appearance */}
<View style={[styles.card, cardBg]}>
  ...
  <Pressable onPress={() => setLangOpen(true)} style={styles.row}>
    <Ionicons name="language-outline" size={20} color={mut.color as string} />
    ...
    <Ionicons name="chevron-forward" size={20} color={mut.color as string} />
        ...
        <View style={[styles.card, cardBg]}>
          ...
          <View style={[styles.row, { gap: 12 }]}>
            <Ionicons name="pricetag-outline" size={20} color={mut.color as string} />
            <TextInput
              value={coupon}
              onChangeText={setCoupon}
              placeholder="Enter coupon code"
              placeholderTextColor={mut.color as string}
              style={[styles.input, fg, { borderColor: cardBg.borderColor as string }]}
        ...
        <View style={[styles.card, cardBg]}>
          ...
          <Pressable style={styles.row} onPress={() => router.push("/login")}>
            <Ionicons name="log-in-outline" size={20} color={mut.color as string} />
        ...
        <Pressable style={styles.primaryBtn} onPress={() => router.push("/(tabs)/browse")}>
      ...
      <Modal visible={langOpen} animationType="slide" transparent onRequestClose={() => setLangOpen(false)}>
        ...
        <View style={[styles.modalSheet, bg]}>
          ...
          {LANGS.map((l) => (
            <Pressable key={l} onPress={() => { setLanguage(l); setLangOpen(false); Alert.alert("Language set (demo)", l); }} style={styles.modalRow}>
              <Text style={[styles.modalText, fg]}>{l}</Text>
              {l === language && <Ionicons name="checkmark" size={18} color={mut.color as string} />}
            </Pressable>
          ))}
          <Pressable onPress={() => setLangOpen(false)} style={[styles.smallBtn, { alignSelf: "flex-end" }]}>
...
const styles = StyleSheet.create({
  ...
  input: { flex: 1, borderWidth: 1, borderRadius: 10, paddingHorizontal: 12, paddingVertical: Platform.OS === "ios" ? 10 : 6 },
  primaryBtn: { marginTop: 4, backgroundColor: "black", borderRadius: 12, padding: 14, alignItems: "center", flexDirection: "row", justifyContent: "center", gap: 8 },
  primaryText: { color: "white", fontWeight: "700" },
  smallBtn: { backgroundColor: "#111", paddingHorizontal: 12, paddingVertical: 10, borderRadius: 10 },
  smallText: { color: "white", fontWeight: "700" },
  dangerBtn: { marginTop: 8, backgroundColor: "#ef4444", borderRadius: 12, padding: 12, alignItems: "center", gap: 6, flexDirection: "row", justifyContent: "center" },
  dangerText: { color: "white", fontWeight: "700" },
  modalBackdrop: { flex: 1, backgroundColor: "rgba(0,0,0,0.35)" },
  modalSheet: { padding: 16, borderTopLeftRadius: 16, borderTopRightRadius: 16, position: "absolute", bottom: 0, left: 0, right: 0, gap: 8 },
  modalRow: { paddingVertical: 12, flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
